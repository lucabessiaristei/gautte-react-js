...
function App() {
	...
	if (loading) {
		return <div className="flex justify-center items-center h-screen text-xl text-gray-600">Loading GTFS data...</div>;
	}
	...
	if (error) {
		return (
			<div className="flex flex-col justify-center items-center h-screen text-xl text-red-600 gap-4">
				...
				<button onClick={() => window.location.reload()} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 cursor-pointer">
					Retry
				</button>
			</div>
		);
	...
	if (!gtfsData.stops || !gtfsData.routes) {
		return <div className="flex justify-center items-center h-screen text-xl text-gray-600">Preparing data...</div>;
	}
